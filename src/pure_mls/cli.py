...
def main() -> None:
	...
	p_add.add_argument("--out-commit", "-c", required=True, help="Path to save the Commit message")
	...
	# remove-member
	p_remove = subparsers.add_parser("remove-member", help="Remove a member from the group")
	p_remove.add_argument("group_state", help="Path to the group state (.state)")
	p_remove.add_argument("leaf_index", type=int, help="Leaf index of the member to remove")
	p_remove.add_argument("--out-commit", "-c", required=True, help="Path to save the Commit message")
	p_remove.add_argument("--out-state", "-o", help="Overwrites group state by default, unless specified here")

	# update-key
	p_update = subparsers.add_parser("update-key", help="Rotate your leaf key for Post-Compromise Security")
	p_update.add_argument("group_state", help="Path to the group state (.state)")
	p_update.add_argument("--out-commit", "-c", required=True, help="Path to save the Commit message")
	p_update.add_argument("--out-state", "-o", help="Overwrites group state by default, unless specified here")

	# apply-commit
	p_apply = subparsers.add_parser("apply-commit", help="Apply a Commit generated by another member")
	p_apply.add_argument("group_state", help="Path to the group state (.state)")
	p_apply.add_argument("commit", help="Path to the Commit message")
	p_apply.add_argument("--out-state", "-o", help="Overwrites group state by default, unless specified here")
	...
	elif args.command == "apply-commit":
		with open(args.group_state, "rb") as f:
			group = MLSGroup.from_bytes(f.read())
