...
from pure_mls.group import MLSGroup, Welcome, GroupSecrets, GroupInfo, _egs_info
from pure_mls.keys import KemKey, SignatureKey
from pure_mls.tree import RatchetTree, LeafNode
import hmac

def _h(s): return bytes.fromhex(s) if s else b""
...
for idx, vec in enumerate(data):
    if vec["cipher_suite"] != 1: continue
    ...
    try:
        ...
        if not gs_bytes:
            print(f"  [-] FAILED: Could not decrypt GroupSecrets")
            continue
        ...
        if not tree:
            print(f"  [-] FAILED: No ratchet_tree in GroupInfo")
            continue
        ...
        if my_index is not None:
            ...
            print(f"  [-] FAILED: My leaf NOT found in tree!")
